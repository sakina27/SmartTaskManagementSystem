...
function TaskManager() {
    ...
    const fetchTasks = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`http://task-service:8080/api/tasks/user/${userId}`);
            setTasks(res.data);
        } catch (err) {
    ...
    const fetchComments = async (taskId) => {
        try {
            const res = await axios.get(`http://comment-service:8080/api/comments/task/${taskId}`);
    ...
    const handleAddComment = async (taskId) => {
        ...
        try {
            await axios.post(`http://comment-service:8080/api/comments`, {
                taskId,
                userId,
                content: commentText
            });
    ...
    const handleDeleteComment = async (commentId, taskId) => {
        ...
        try {
            await axios.delete(`http://comment-service:8080/api/comments/${commentId}`);
            await fetchComments(taskId);
        } catch (err) {
    ...
    const handleUpdateComment = async (comment, taskId) => {
        try {
            await axios.put(`http://comment-service:8080/api/comments/${comment.id}`, {
                ...comment,
    ...
    const handleFetchFromGoogleCalendar = async () => {
        ...
        try {
            await axios.get(`http://task-service:8080/api/tasks/fetch-google-calendar`, {
    ...
    const handleAnalyzeDescription = async () => {
        ...
        try {
            setLoading(true);
            const res = await axios.post('http://nlp-service:5001/text-to-task', { text: description });
    ...
    const startRecording = async () => {
        try {
            ...
            mediaRecorderRef.current.onstop = async () => {
                ...
                try {
                    setLoading(true);
                    const nlpRes = await axios.post("http://nlp-service:5001/audio-to-task", formData);
    ...
    const toggleTaskStatus = async (taskId, currentStatus) => {
        ...
        try {
            await axios.patch(`http://task-service:8080/api/tasks/${taskId}`, { status: newStatus });
            fetchTasks();
        } catch (err) {
